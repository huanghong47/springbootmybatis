...
//                }
//            }
//            this.response401(request, response, "token认证失败");
//            return false;
//        }
    ...
    protected boolean isLoginAttempt(ServletRequest request, ServletResponse response) {
        ...
        return ParamUtils.isNotBlank(token);
    }
    ...
    protected boolean executeLogin(ServletRequest request, ServletResponse response)  {
        // 拿到当前Header中Authorization的AccessToken(Shiro中getAuthzHeader方法已经实现)
        String tokenbs = this.getAuthzHeader(request);//此token是被后台处理过的
    ...
    private boolean refreshToken(ServletRequest request, ServletResponse response) {
        ...
        String token = this.getAuthzHeader(request);
        // 获取当前Token的帐号信息
        String account = JwtUtil.getClaim(token, Constant.JWTACCOUNT);
    ...
    private void response401(ServletRequest req, ServletResponse resp, String msg) {
        ...
        try {
            ...
            String data = JsonConvertUtil.objectToJson(new ResponseVO(ResponseVO.CODE_USER_PERMISSION_FAILED, ResponseVO.MESSAGE_USER_PERMISSION_FAILED +":"+ msg, -1L,new Object()));
            out.append(data);
        } catch (IOException e) {
